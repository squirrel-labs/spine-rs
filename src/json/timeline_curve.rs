...
use serde::de::{Error as SerdeError, Visitor, SeqAccess, value::SeqAccessDeserializer};
use std::fmt;
...
impl<'a> Visitor<'a> for TimelineCurveVisitor {
    ...
    fn visit_str<E>(self, value: &str) -> Result<Self::Value, E> where E: SerdeError {
        match value {
            ...
            _ => Err(SerdeError::custom(format!("Timeline curve must be either linear, stepped or an array")))
        }
    }
    ...
    fn visit_seq<S>(self, visitor: S) -> Result<Self::Value, S::Error> where S: SeqAccess<'a> {
